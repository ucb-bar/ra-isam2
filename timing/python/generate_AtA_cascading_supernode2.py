...
def write_factors(fout, factors, seq, seq_pos):
    ...
    all_factors = {"vars": [], "matrix": []}
    for k, split_index in enumerate(sorted_split_indices):
        all_factors["vars"].extend(factors[split_index]["vars"])
        all_factors["matrix"].extend(factors[split_index]["matrix"])

    nfactors = len(all_factors["vars"])
    fout.write(f"int nfactors = {nfactors};\n\n")

    for i in range(nfactors):
        v = all_factors["vars"][i]
        m = all_factors["matrix"][i].astype(np.float32)
    ...
    fout.write(f"const int factor_heights[] = {{\n")
    for i in range(nfactors):
        fout.write(f"factor{i}_height, ")
    fout.write("};\n")
    fout.write(f"const int factor_widths[] = {{\n")
    for i in range(nfactors):
        fout.write(f"factor{i}_width, ")
    fout.write("};\n")
    fout.write(f"const int* factor_ridx[] = {{\n")
    for i in range(nfactors):
        fout.write(f"factor{i}_ridx, ")
    fout.write("};\n")
    fout.write(f"const float* factor_data[] = {{\n")
    for i in range(nfactors):
    ...
    for split_index in sorted_split_indices:
        node_factors[split_index] = {"factors": set(), "factor_info": []}
    ...
    for i in range(nfactors):
        ...
        for j, var in enumerate(v):
            if var == last_var:
                continue
            for k, split_index in enumerate(sorted_split_indices):
                if k == len(sorted_split_indices) - 1 or (var >= split_index and var < sorted_split_indices[k + 1]):
                    if i not in node_factors[split_index]["factors"]:
                        node_factors[split_index]["factors"].add(i)
                        node_factors[split_index]["factor_info"].append([i, start_row])
                    break
            start_row += seq[var]

    for k, split_index in enumerate(sorted_split_indices):
        node_nfactors = len(node_factors[split_index]["factor_info"])
        fout.write(f"const int node{k}_nfactors = {node_nfactors};\n")
        fout.write(f"const int node{k}_factors[] = {{")
        for i, info in enumerate(node_factors[split_index]["factor_info"]):
            fout.write(f"{info[0]}, ")
        fout.write("};\n")
        fout.write(f"const int node{k}_factor_starts[] = {{")
        for i, info in enumerate(node_factors[split_index]["factor_info"]):
            fout.write(f"{info[1]}, ")
        fout.write("};\n")
        fout.write("\n")
...
def write_neg_factor(fout, seq, seq_pos, sel_ancestors, supernode_heights, supernode_ridx):
    sorted_split_indices = sorted(supernode_ridx.keys())
    node_factors = {}
    for split_index in sorted_split_indices:
        node_factors[split_index] = {"factors": set(), "factor_info": []}
    for k, split_index in enumerate(sorted_split_indices):
    ...
    print(supernode_ridx)
    print(sel_ancestors)
    last_var = len(seq) - 1
    for i, node_index in enumerate(sorted_split_indices):
        v = all_factors["vars"][i]
        start_row = 0;
        for j, var in enumerate(v):
            if var == last_var:
...
def write_block_sparse_matrix(fout, matrix, matrix_name, seq, seq_pos, sel_ancestors, supernode_heights, supernode_ridx):
    ...
    for i, start_index in enumerate(supernode_indices):
        ...
        dense_col = np.zeros((node_height, node_width))
...
if __name__ == "__main__":
    ...
    if outfile is not None:
        ...
        with open(outfile, "w") as fout:
            ...
            write_neg_factor(fout, seq, seq_pos, sel_ancestors, supernode_heights, supernode_ridx)
